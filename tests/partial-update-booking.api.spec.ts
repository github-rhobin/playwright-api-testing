...
test(
  '[PATCH] Partial Update Booking Details - Valid Token',
  { tag: ['@positive'] },
  async ({ bookingClient }, testInfo) => {
    /**
    ...
    await test.step('Validation', async () => {
      ...
      expect(patchResponseDetails.isResponseSuccessful, 'Should be a Successful Response').toBe(true);

      // Strict check for a specific code
      expect(patchResponseDetails.status, 'Status should be "200"').toBe(200);

      // Strict check for a specific code
      expect(patchResponseDetails.statusText, 'Status Text should be "OK"').toBe('OK');
...
test(
  '[PATCH] Partial Update Booking Details - Invalid/No Token',
  { tag: ['@negative'] },
  async ({ bookingClient }, testInfo) => {
    /**
     * Since restful-booking heroku app auto clears bookings
    ...
    await test.step('Validation', async () => {
      ...
      // Flexible check for any success code (200-299)
      expect(patchResponseDetails.statusText, 'Status Text should be "Forbidden"').toBe('Forbidden');
    });
  }
);
